...
public class Crypto {
    ...
    private byte[] encrypt(byte[] message, Key keySpec, String algorithm ) throws InvalidAlgorithmParameterException, InvalidKeyException, BadPaddingException, IllegalBlockSizeException, NoSuchPaddingException, NoSuchAlgorithmException {
    ...
    byte[] makeKey() throws NoSuchAlgorithmException {
    ...
    public PublicKey get_public_key (String keyStore_location, char[] keyStore_password, String alias) throws KeyStoreException, IOException, CertificateException, NoSuchAlgorithmException, UnrecoverableKeyException {
    ...
    public PrivateKey get_private_key(String keyStore_location, char[] keyStore_password, String alias, char[] password) throws IOException, KeyStoreException, CertificateException, NoSuchAlgorithmException, UnrecoverableKeyException {
        KeyStore keyStore = KeyStore.getInstance(KeyStore.getDefaultType());
        FileInputStream fis = new FileInputStream(keyStore_location);
        keyStore.load(fis, keyStore_password);
        PrivateKey private_key = (PrivateKey)keyStore.getKey(alias, password);
        return private_key;
    }

    public byte[] encrypt_key_rsa(byte[] message, PublicKey key) throws BadPaddingException, InvalidKeyException, NoSuchAlgorithmException, IllegalBlockSizeException, NoSuchPaddingException, InvalidAlgorithmParameterException {
    ...
    byte[] readFile(String fileLocation, byte[] key) throws IOException, NoSuchPaddingException, NoSuchAlgorithmException, InvalidAlgorithmParameterException, InvalidKeyException, BadPaddingException, IllegalBlockSizeException {
        ...
        byte[] decText = decrypts.doFinal(encrypted_text);
        return decText;
    ...
    private byte[] generateRandomString(int n) {
